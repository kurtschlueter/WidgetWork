 // CampaignBreakDownWheel object takes in two properties; an elementID to identify the canvas container in the DOM, and a json object that contains the data that will populate the wheel along with the settings.
function CampaignBreakDownWheel(elementDOM, settingsJSON) {

  this.minSecLength = 4; // min section length that triggers line attachment
  this.specialPositioningStatsHolder = {};
...
CampaignBreakDownWheel.prototype.setScreenForPixels = function() {
...
CampaignBreakDownWheel.prototype.level1PercentageAdjuster  = function() {
...
CampaignBreakDownWheel.prototype.recursivelyLoopAndGrabDataForCalculationsAndOrdering  = function(level, currentPercentScopeView, currentPercentScopeData, radiusScope, obj) {
...
CampaignBreakDownWheel.prototype.setSpecialExtensionProperties  = function(level, section) {
  var currentLevel = this.layersWithSectionData_a_o[level];
...
CampaignBreakDownWheel.prototype.setLevelProperties  = function(level) {
  var currentLevel = this.layersWithSectionData_a_o[level];
...
CampaignBreakDownWheel.prototype.setSectionLineAttachmentProperties  = function(level, section) {
...
CampaignBreakDownWheel.prototype.generateSectionColor  = function(level, section) {
  var currentLevel = this.layersWithSectionData_a_o[level];
  var currentSection = currentLevel.sections[section];
  ...
  if (currentLevel.colorSchemeType_s == 'custom') {
    ...
    var newLightness = (parentSectionLightness + (7 * (currentSection.indexWithRespectToParentSection_i + 1))).toString();
...
CampaignBreakDownWheel.prototype.calculateSectionArcGlobalPercentage  = function(level, section) {
  var currentLevel = this.layersWithSectionData_a_o[level];
  var currentSection = currentLevel.sections[section];
...
CampaignBreakDownWheel.prototype.calculateRadialStarts  = function(level, section) {
  var currentLevel = this.layersWithSectionData_a_o[level];
  var currentSection = currentLevel.sections[section];
...
CampaignBreakDownWheel.prototype.calculateRadialEnds  = function(level, section) {
  var currentLevel = this.layersWithSectionData_a_o[level];
  var currentSection = currentLevel.sections[section];
...
CampaignBreakDownWheel.prototype.calculateGridPositioning = function(level, section, numberOfSections) {
...
CampaignBreakDownWheel.prototype.calculateLineCoordinatesOutAndUpOrDown = function(level, section, numberOfSections) {
...
CampaignBreakDownWheel.prototype.calculateLineCoordinatesOutAndLeftOrRight = function(level, section, numberOfSections) {
...
CampaignBreakDownWheel.prototype.animationCycler = function(sectionIDs, sectionPercentInterval) {
...
CampaignBreakDownWheel.prototype.pastePreviouslyPasted = function(sectionIDs) {
...
CampaignBreakDownWheel.prototype.pastePieArc = function(levelID, sectionID, tempRadialEnd, radiusPop) {
...
CampaignBreakDownWheel.prototype.pastePieText = function(levelID, sectionID) {
  var currentLevel = this.layersWithSectionData_a_o[levelID];
  var currentSection = currentLevel.sections[sectionID];
  var toPaste;
  var xTouches = 0;
...
CampaignBreakDownWheel.prototype.pasteCenterOptions = function(levelID, sectionID) {
...
CampaignBreakDownWheel.prototype.pasteLinesForExtention = function(levelID, sectionID) {
  var currentLevel = this.layersWithSectionData_a_o[levelID];
  var currentSection = currentLevel.sections[sectionID];
  this.ctx.beginPath();
...
CampaignBreakDownWheel.prototype.pasteLineExtensionTexts = function(levelID, sectionID) {
	// debugger
	var currentLevel = this.layersWithSectionData_a_o[levelID];
	var currentSection = currentLevel.sections[sectionID];
	var buffer = 5;
...
// console.log('current level', currentLevel)
	...
	// If line attachment equals true and the previos container (level 1) can fit the icon (is greater than minimum arc length)
	if (currentSection.lineAttachment_b == true && currentSection.previousContainerPercentageScopeView_i > this.minSecLength) {
	    ...
	    for (var i = 0 ; i < currentSection.attachedPrintOptions_s.length ; i++) {
			...
			// if center of current section is on left side of wheel
			if ((currentSection.radialStart + currentSection.radialEnd) / 2 >= Math.PI/2 && (currentSection.radialStart + currentSection.radialEnd) / 2 < Math.PI*3/2) {

				this.ctx.font = '12pt MetropolisRegular';
			...
			// if center of current section is on right side of wheel
			} else {
	...
	// If line attachment equals true and the previos container (level 1) cannot fit the icon (is <= than minimum arc length)
	// This code is complete shit. Hardcoded shit. For now it works but it will have to change. So embarrasing
	    ...
	    if ((currentSection.radialStart + currentSection.radialEnd) / 2 >= Math.PI / 2 && (currentSection.radialStart + currentSection.radialEnd) / 2 < 3 *  Math.PI / 2) {

		    xPositioningIcon = currentSection.lineCoords_a_i[2][0] - 14;
		    yPositioningIcon = currentSection.lineCoords_a_i[2][1] + 22;
	    ...
	    if ( currentSection.starterStatus_b == true) {
	        ...
	        this.specialPositioningStatsHolder.startingDataPlacementY = yPositioningIcon;
	        this.specialPositioningStatsHolder.index = 0;

	        if ((currentSection.radialStart + currentSection.radialEnd) / 2 >= Math.PI / 2 && (currentSection.radialStart + currentSection.radialEnd) / 2 < 3 *  Math.PI / 2) {
	            this.specialPositioningStatsHolder.startingDataPlacementX = xPositioningIcon - 28 - 30;
	        } else {
	            this.specialPositioningStatsHolder.startingDataPlacementX = xPositioningIcon + 28;
	        }
	    }

	    this.ctx.beginPath();
	    this.ctx.font = this.layersWithSectionData_a_o[0].textFont_s;
	    // console.log(this.layersWithSectionData_a_o[0].textFont_s)
	    this.ctx.fillStyle = this.layersWithSectionData_a_o[0].sections[currentSection.previousContainerSectionScope_i - 1].color_s;

	    // If center of current section is on left side of wheel
	    if ((currentSection.radialStart + currentSection.radialEnd) / 2 >= Math.PI/2 && (currentSection.radialStart + currentSection.radialEnd) / 2 < Math.PI*3/2) {

			// If starter. The line will extend out of the first section in the second level. Baby I need knowww
			if ( currentSection.starterStatus_b == true) {

				this.ctx.textBaseline = "bottom";
				this.ctx.font = this.layersWithSectionData_a_o[0].sections[currentSection.previousContainerSectionScope_i - 1].textFont_s;
				...
				this.ctx.arc(this.specialPositioningStatsHolder.startingDataPlacementX, this.specialPositioningStatsHolder.startingDataPlacementY - 28, 5, 0, Math.PI*2, true);
				this.ctx.fillStyle = currentSection.color_s;
				this.ctx.fill();

				toPasteNext = (Math.round(currentSection.globalPercentageArcData_i * 100) / 100).toString() + "%";
				//        console.log(toPasteNext)
				this.ctx.font = '10pt MetropolisLight';
				this.ctx.textBaseline = "left";
				this.ctx.fillStyle = 'black';
				this.ctx.fillText(toPasteNext, this.specialPositioningStatsHolder.startingDataPlacementX + 25, this.specialPositioningStatsHolder.startingDataPlacementY - 28 + 7);

			} else {
				var yShift1 = this.specialPositioningStatsHolder.startingDataPlacementY - 27 + 13.5 * this.specialPositioningStatsHolder.index;
		        this.ctx.arc(this.specialPositioningStatsHolder.startingDataPlacementX, yShift1, 5, 0, Math.PI*2, true);
		        this.ctx.fillStyle = currentSection.color_s;
		        this.ctx.fill();
		        ...
		        this.ctx.textBaseline = "left";
		        this.ctx.fillStyle = 'black';
		        this.ctx.fillText(toPasteNext, this.specialPositioningStatsHolder.startingDataPlacementX + 25, this.specialPositioningStatsHolder.startingDataPlacementY + 14 * this.specialPositioningStatsHolder.index - 23);
		    }

		// If center of current section is on right side of wheel
		} else {
			console.log('cureent section text', currentSection)
			// If starter. The line will extend out of the first section in the second level. Baby I need knowww
			if ( currentSection.starterStatus_b == true) {
				...
				this.ctx.arc(this.specialPositioningStatsHolder.startingDataPlacementX, this.specialPositioningStatsHolder.startingDataPlacementY + 10, 5, 0, Math.PI*2, true);
				this.ctx.fillStyle = currentSection.color_s;
				this.ctx.fill();
				...
				this.ctx.textBaseline = "left";
				this.ctx.fillStyle = 'black';
				this.ctx.fillText(toPasteNext, this.specialPositioningStatsHolder.startingDataPlacementX + 25, this.specialPositioningStatsHolder.startingDataPlacementY + 2);

			} else {

				this.ctx.arc(this.specialPositioningStatsHolder.startingDataPlacementX, this.specialPositioningStatsHolder.startingDataPlacementY + 11.5 + 13.5 * this.specialPositioningStatsHolder.index, 5, 0, Math.PI*2, true);
				this.ctx.fillStyle = currentSection.color_s;
				this.ctx.fill();
				...
				this.ctx.textBaseline = "left";
				this.ctx.fillStyle = 'black';
				this.ctx.fillText(toPasteNext, this.specialPositioningStatsHolder.startingDataPlacementX + 25, this.specialPositioningStatsHolder.startingDataPlacementY + 14 * this.specialPositioningStatsHolder.index +14);
			}
		}

		this.specialPositioningStatsHolder.index = this.specialPositioningStatsHolder.index + 1;
		this.ctx.textBaseline = "alphabetic";
		this.ctx.closePath();
	}
}

CampaignBreakDownWheel.prototype.drawWheel = function() {
